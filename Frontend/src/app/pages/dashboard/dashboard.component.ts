...
export class DashboardComponent implements OnInit {
  INITIAL_CONTENTS = [
    ...
    { key: 'DAO tokenAddress', prop: 'Token address', value: 'Loading...' },
    ...
    { key: 'WETH token address', prop: 'Token address', value: 'Loading...' },
    { key: 'WETH token balance', prop: 'User balance', value: 'Loading...' },
    { key: 'Vault address', prop: 'Vault address', value: 'Loading...' },
    { key: 'Vault balance', prop: 'Vault balance', value: 'Loading...' },
  ];
  ...
  private updateValues() {
    ...
    this.blockchainService.WETHtokenAddress().then((result) => {
      const itemIndex = this.pageContents.findIndex(
        (obj) => obj.key === 'WETH token address'
      );
      if (itemIndex >= 0) this.pageContents[itemIndex].value = result;
    });
    this.blockchainService.vaultAddress().then((result) => {
      const itemIndex = this.pageContents.findIndex(
        (obj) => obj.key === 'Vault address'
      );
      if (itemIndex >= 0) this.pageContents[itemIndex].value = result;
    });
  }
  ...
  private updateWETHTokenBalance() {
    this.blockchainService.WETHtokenBalance().then((result) => {
      const itemIndex = this.pageContents.findIndex(
        (obj) => obj.key === 'WETH token balance'
      );
      if (itemIndex >= 0) this.pageContents[itemIndex].value = result;
    });
  }

  private updateVaultBalance() {
    this.blockchainService.vaultBalance().then((result) => {
      const itemIndex = this.pageContents.findIndex(
        (obj) => obj.key === 'Vault balance'
      );
      if (itemIndex >= 0) this.pageContents[itemIndex].value = result;
    });
  }
  ...
  depositToken(amountStr: string) {
    const amount = Number(amountStr);
    this.blockchainService.depositToken(amount);
  }
}
